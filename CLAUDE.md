...
- **`docs/BDD.md`** — user stories + scénarios Gherkin (comportement utilisateur/dashboard : auth, navigation, portefeuilles, bots, visualisation). Chaque scénario = un test. **Tout nouveau comportement front ajoute un scénario ici.**
...
5. **Stratégies = fonctions pures** : `decide(context): Signal` (BUY/SELL/HOLD), déterministe. Contrat et fiches détaillées dans `docs/strategies/`. Clés disponibles : `ma_crossover`, `rsi`, `macd`, `bollinger_bands`, `momentum_roc`, `donchian_breakout`, `buy_and_hold` (benchmark), `candle_streak`, `trend_filter`.
